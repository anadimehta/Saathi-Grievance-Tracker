...
export default function Dashboard() {
  ...
  // Log to help troubleshooting rendering
  useEffect(() => {
  ...
  const handleGo = () => {
    if (selectedDistrict) {
      console.log("Navigating to district:", selectedDistrict);
      router.push(`/district/${selectedDistrict}`);
    }
  };
  ...
  return (
    <div className="space-y-8 px-6 md:px-12 lg:px-20 py-8 bg-[#f0f7ff] min-h-screen">
      ...
      <Card className="mt-8 w-full max-w-sm mx-auto shadow-lg bg-blue-100 border border-gray-300">
        ...
        <CardContent className="flex flex-col items-center gap-4">
          <Select value={selectedDistrict} onValueChange={setSelectedDistrict}>
            ...
            <SelectContent className="bg-white shadow-lg rounded-lg">
              {districts.map((district) => (
                <SelectItem key={district} value={district} className="px-4 py-2 hover:bg-blue-200">
                  {district}
                </SelectItem>
              ))}
            </SelectContent>
      ...
      <div className="flex justify-center">
        <Image
          src="/UP1.png" // Ensure this image exists in the public folder
          alt="Uttar Pradesh Map"
          width={800}
          height={400}
        />
